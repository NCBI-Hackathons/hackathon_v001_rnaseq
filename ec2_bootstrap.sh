...
sudo ln -s `pwd`/hisat-0.1.2-beta-Linux_x86_64/hisat /usr/local/  || exit 0;
sudo ln -s `pwd`/hisat-0.1.2-beta-Linux_x86_64/hisat-build /usr/local/bin || exit 0;
# Download and make SAMTools
sudo apt-get install -f libncurses5-devn || { echo "curses installation failed" ; exit 1; }
